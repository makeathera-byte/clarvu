/**
 * Simple script to generate AI summary
 * Reads env vars from process.env directly (assumes they're set)
 */

const { createClient } = require('@supabase/supabase-js');
const Groq = require('groq-sdk');

// Get env vars from process.env (assumes they're already loaded)
  ...
  const compressedLogs = logs
    .map((log) => {
      ...
      const duration = log.end_time
        ? Math.round((new Date(log.end_time).getTime() - new Date(log.start_time).getTime()) / (1000 * 60))
        : 'ongoing';
  ...
  try {
    const completion = await groq.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: 'You are a productivity coach analyzing someone\'s daily activity log. Generate a concise, insightful daily summary. Keep it under 150 words.',
        },
