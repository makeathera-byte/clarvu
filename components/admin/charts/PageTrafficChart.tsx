...
export function PageTrafficChart({ data }: PageTrafficChartProps) {
  const { theme } = useTheme();
  const isDark = theme === "dark";
  const barColor = isDark ? "#ffffff" : "#3b82f6"; // Blue for light mode
  
  if (!data || data.length === 0) {
    return (
      <Card className="border-border/40 shadow-sm rounded-xl">
        <CardHeader>
          <CardTitle className="text-lg">Traffic by Page</CardTitle>
        </CardHeader>
        <CardContent>
