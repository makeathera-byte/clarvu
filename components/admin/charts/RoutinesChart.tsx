...
interface RoutinesChartProps {
  data: Array<{ date: string; routines: number }>;
}

export function RoutinesChart({ data }: RoutinesChartProps) {
  ...
  if (!data || data.length === 0) {
    return (
      <Card className="border-border/40 shadow-sm rounded-xl">
        <CardHeader>
          <CardTitle className="text-lg">Routines Generated (Last 7 Days)</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex items-center justify-center h-[300px] text-muted-foreground">
            No routines generated
          </div>
        </CardContent>
      </Card>
    );
